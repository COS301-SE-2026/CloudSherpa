...
export default function ResourceManager() {
    ...
    const handleConfirm = () => {
        ...
        if (pending.direction === "enable") {
            setInactive((previous) =>
                previous.filter((resource) => resource.id !== pending.resource.id)
            );
    ...
    const confirmationText =
        pending?.direction === "enable"
            ? "Are you sure you want resource to be active?"
            : "Are you sure you want resource to be inactive?";

    return (
        <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-background text-foreground">
            <h2 className="text-lg font-medium tracking-tight mb-8 text-foreground">
                {" "}
                Resource Manager{" "}
            </h2>
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <Card className="w-96 border-border">
                            ...
                            <div className="flex items-center justify-between gap-2">
                                <Input
                                    ...
                                    onChange={(clickSearch) =>
                                        setInactiveSearch(clickSearch.target.value)
                                    }
                        ...
                        <CardContent className="p-2 min-h-96">
                                ...
                                {inactiveFiltered.map((resource) => (
                                    ...
                                    >
                                        <button
                                            ...
                                            className="flex-1 text-left bg-transparent border-none text-inherit text-sm"
                                        >
                                            {" "}
                                            {resource.name}
                                        </button>
                                        ...
                                        <button
                                            onClick={(clickingAdd) => {
                                                clickingAdd.stopPropagation();
                                                handleAdd(resource);
                                            }}
                                            className="text-base leading-none ml-2 text-sidebar-accent hover:text-ring"
                                        >
                                            {" "}
                                            +
                                        </button>
                        ...
                        <CardHeader className="px-3 py-2 border-b border-border">
                            <div className="flex items-center justify-between gap-2">
                                <Input
                                    ...
                                    value={activeSearch}
                                    onChange={(clickSearch) =>
                                        setActiveSearch(clickSearch.target.value)
                                    }
                                    className="h-5 text-xs border-none shadow-none px-1 bg-transparent focus-visible:ring-0 placeholder:text-neutral-500 text-foreground"
                                />
                    ...
                    <Card className="w-80 text-center p-8 border-border bg-background">
                        ...
                        <div className="flex gap-3 justify-center">
                            <button
                                ...
                                className="px-6 py-2 text-sm font-medium rounded transition-colors border border-muted-foreground/30 text-foreground hover:bg-muted"
                            >
                                {" "}
                                cancel
                            </button>
                            ...
                            <button
                                ...
                                className="px-6 py-2 text-sm font-medium rounded transition-colors text-primary-foreground bg-primary hover:bg-primary/90"
                            >
                                {" "}
                                confirm
                            </button>
