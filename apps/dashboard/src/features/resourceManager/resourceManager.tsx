...
import { getAwsAccountResources, ResourceStatus, updateAwsResourceStatus } from "@/lib/fetch/aws-connection-api";
    ...
    try {
      ...
      setResource(
        resources.map(resource => ({
          ...
          region: "none",
          tag: Object.entries(resource.tags).map(
            ([key, value]) => `${key}: ${value}`
          ),
          status:
      ...
      setResource((previous) =>
        previous.map((resource) =>
          resource.id === id
            ? {
              ...resource,
              status: newStatus === ResourceStatus.ACTIVE
                ? "active"
                : "inactive",
            }
            : resource
