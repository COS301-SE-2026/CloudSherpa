            ...
            <CardContent className = "flex flex-col gap-4">
                <div className = "flex flex-row gap-2">
                    <div className = "relative flex-1">
                        <Search className = "absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" strokeWidth = {1.75}/>

                        <Input value = {search} onChange = {(change) => onSearchChange(change.target.value)} placeholder = "Search..." className = "pl-8"/>
                        ...
                        forFilteredEntries.map((entry) => (
                            <div key = {entry.id} className = "flex flex-col gap-1.5">
                                ...
                                <div className = "flex flex-row items-center gap-3">
                                    ...
                                    <span className = "text-xs text-muted-foreground w-9 text-right"> {entry.percent}% </span>
                                </div>
                            </div>
                        ))
                    )}
