...
function upsertMetric(seriesByKey: MetricStore["seriesByKey"], metric: Metric): MetricStore["seriesByKey"] {
  const key = metricSeriesKey(metric);
  const existingSeries = seriesByKey[key] || [];
  const timeMs = new Date(metric.timestamp).getTime();
  const newPoint: [number, number] = [timeMs, metric.value];
  return {
    ...seriesByKey,
    [key]: [...existingSeries, newPoint],
  };
}
