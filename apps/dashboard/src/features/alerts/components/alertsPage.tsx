...
export function AlertsPage(){
    ...
    const forFilters = useMemo(() => {
        ...
        return alerts.filter((alert) => {
            ...
            const matchesSearch = alert.title.toLowerCase().includes(wordSearched) || (alert.message?.toLowerCase().includes(wordSearched) ?? false);
    ...
    return(
        <div className = "mx-auto max-w-6xl px-6 py-8">
            <header className = "mb-6">
                <h1 className = "mb-4 text-2xl font-semibold tracking-tight text-foreground"> Alerts </h1>

                <div className = "mb-4 flex flex-wrap items-center gap-3">
                    <Input className = "h-9 w-64" placeholder = "Search alerts" value = {search} onChange = {(change) => setSearch(change.target.value)}/>
                ...
                <p className = "text-sm text-muted-foreground"> {activeCount} active of {alerts.length} alerts </p>
            </header>

                {loading && (
                    <Card>
                        <CardContent className = "py-8 text-center text-sm text-muted-foreground"> Loading alerts </CardContent>
                    </Card>
                )}
