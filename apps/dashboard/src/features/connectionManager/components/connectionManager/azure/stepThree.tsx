...
export default function StepThreeAzure({
    displayName, resources = [], onComplete, onBack, ingestionPeriod = 60,
    ...
    const handlingSubmit = async (event : React.SubmitEvent<HTMLFormElement>) => {
        ...
        try{
            ...
            setErrors( forError instanceof Error ? forError.message : "Unable to create an Azure connection");
        }finally{
            setForSaving(false);
        }
    };
    ...
    return(
        <StepThree heading = "Select instances" description = "Select the instance you want CloudSherpa to monitor" onSubmit = {handlingSubmit} onBack = {onBack || (() => {})} forSaving = {forSaving} forErrors = {errors}>
