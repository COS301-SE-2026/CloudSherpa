...
export default function WizardSetupAzure() {
    ...
    const [wizardData, setWizardData] = useState<DataForWizard>({
        displayName : "Azure Connection", ingestionPeriod : 60, servicesSelected : [], resources : [],
    });

    const handleStepOneNext = (data : {displayName : string}) => {
        setWizardData((previous) => ({
            ...previous, displayName : data.displayName,
        }));
    ...
    const handleStepTwoNext = (data : {
        servicesSelected : string[]; resources : ResourceDetail[];
    }) => {
        setWizardData((previous) => ({
            ...previous, servicesSelected : data.servicesSelected, resources : data.resources,
        }));
    ...
    return(
        <>
            ...
            {step === 2 && (
                <StepTwoAzure onNext = {handleStepTwoNext} onBack = {handleBack}/>
            )}

            {step === 3 && (
                <StepThreeAzure displayName = {wizardData.displayName}
