...
const hardCodedConn : Connections[] = [
  {id : 1 , name : "Connection 1", detail : "details about conn", provider : "AWS", resource : 3, status : "active"},

  {id : 2, name : "Connection 2", detail : "details about connection", provider : "Azure", resource : 5, status : "active"},

  {id : 3, name : "Connection 3", detail : "details about connection", provider : "GCP", resource : 0, status : "inactive"},
    ...
    <div
      ...
      <div className = "flex items-center justify-between mb-6">
        <div className = "flex items-center gap-3">
          ...
          <Button variant = "ghost" size = "icon" className = "text-muted-foreground hover:text-foreground h-8 w-8"> <ArrowLeft size = {18}/> </Button>
        ...
        <Button
          onClick = {handleAdd}
          className = "text-sm px-3 py-1.5 h-auto bg-primary hover:bg-primary/90"> + add
        </Button>
      ...
      <div className = "flex items-center justify-between mb-6">
        ...
        <div className = "flex items-center gap-2">
          ...
          <Button variant = "ghost" size = "icon" className = "text-muted-foreground hover:text-foreground h-8 w-8"> <Search size = {16}/> </Button>
          <Button variant = "ghost" size = "icon" className = "text-muted-foreground hover:text-foreground h-8 w-8"> <SlidersHorizontal size = {16}/> </Button>
          ...
          <Card 
            ...
            <CardContent className = "p-4">
              <div className = "flex items-center justify-between mb-3">
                <span className = {`text-xs font-medium px-2.5 py-0.5 rounded-full ${badges(connection.provider)}`}> {connection.provider} </span>
                ...
                <div className = "flex items-center gap-1">
                  <Button
                    ...
                    variant = "ghost"
                    size = "icon"
                    className = "h-6 w-6 text-muted-foreground hover:text-destructive transition-colors"> <Trash2 size={13} />
                  </Button>

                  <Button
                    variant = "ghost" 
                    size = "icon" 
                    className = "h-6 w-6 text-muted-foreground hover:text-foreground transition-colors"> <MoreVertical size = {13}/>
                  </Button>

                </div>
              </div>

              <p className = "text-sm font-medium text-foreground mb-0.5"> {connection.name} </p>

              <p className = "text-xs text-muted-foreground mb-4"> {connection.detail} </p>

              <div className = "flex flex-col gap-3 pt-3">
                <Separator className = "bg-border"/>

                <div className = "flex items-center justify-between">
                  <span className = "text-xs text-muted-foreground"> {connection.resource} resource{connection.resource !== 1 ? "s" : ""}</span>

                  {connection.resource === 0 ? (
                    <Badge variant = "secondary" className = "text-xs px-2 py-0.5"> inactive </Badge> ) : (
                    <Badge className = "bg-success/20 text-success border-success/20 hover:bg-success/30 text-xs px-2 py-0.5"> active </Badge>
                  )}
