...
import {
    ...
    editThreshold,
    fetchThresholds,
} from "@/features/thresholds/thresholds";
...
export function useThresholds(resourceId?: string): RequestForThreshold {
    ...
    const createThreshold = useCallback(async (forPayload: CreateThresholdRequest) => {
    ...
    const updateThreshold = useCallback(async (id: string, forPayload: UpdateThresholdRequest) => {
    ...
    const removeThreshold = useCallback(async (id: string) => {
        ...
        setThresholds((previous) =>
            previous.filter((forThreshold) => forThreshold.thresholdId !== id)
        );
    }, []);
