...
const Aws_Troubleshooting_INS = creatingIns([
    {
        ...
        details: [
            "S3 bucket name - The user should get it from their own AWS environment. They need to identify existing S3 buckets that they own and have to create a new rule for this purpose. The name must be globally unique.",
            "Export name - The user can create this themselves. It is an identifier for the export they are setting up. The AWS documentation provides rules for valid names, such as using only alphanumeric characters, hyphens and underscores.",
...
const Gcp_Troubleshooting_INS = creatingIns([
...

    {
        ...
        details: [
            "Verify the service accounts IAM roles - in the GCP console, go to the IAM section and check which roles are assigned to the specific service account at the organization, folder and project levels.",
            "Confirm minimum required permissions - ensure the service account has the relevant roles listed in step 2 of the GCP setup wizard or a custom role that includes list permissions.",
...
const Azure_Troubleshooting_INS = creatingIns([
    {
        ...
        details: [
            "Check the service principal's role assignment - navigate to the azure portal -> your subscription/management group -> access control (IAM) -> role assignments.",
            ...
            "Test the query - Use the Azure resource graph explorer in the portal which is signed in as the service principal (or using its credentials) to run a test query. This will confirm whether the principal can fetch any resource data at all.",
