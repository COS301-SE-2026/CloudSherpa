...
export function BudgetSection(){
        ...
        try{
            if(editing){
                await updateBudget(editing.budget_id, {
                    amount : payload.amount, currency : payload.currency, window_days : payload.window_days, enabled : payload.enabled,
                });
    ...
    return(
        <>
            ...
            {loading && (
                <Card>
                    <CardContent className = "py-6 text-center text-sm text-muted-foreground"> Loading budgets </CardContent>
                </Card>
            )}

            {forError && (
                <Card>
                    <CardContent className = "py-6 text-center text-sm text-destructive"> {forError} </CardContent>
                </Card>
            )}
            ...
            <AlertDialog open = {deleteBudget !== null} onOpenChange = {(change) => !change && setDeleteBudget(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription> This will permanently delete the {deleteBudget?.scope} budget of {deleteBudget?.currency} {deleteBudget?.amount}. </AlertDialogDescription>
                    </AlertDialogHeader>
