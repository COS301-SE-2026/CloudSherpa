...
function helperForColumns({edit, toggleEnabled, onDelete} : Columns) : ColumnDef<Budget>[]{
    return[
        {id : "enabled", header : () => "ENABLED", enableSorting : false, cell : ({row}) => (
            <Switch checked = {row.original.enabled} onCheckedChange = {(checked) => toggleEnabled(row.original, checked)}/>
        ),},

        {accessorKey : "scope", header : () => "SCOPE", cell : ({row}) => {
            const mutedBudget = !row.original.enabled;

            return(
                <span className = {mutedBudget ? "text-muted-foreground" : "text-foreground"}> {row.original.scope} </span>
            );
        },},

        {accessorKey : "budget", header : () => "BUDGET", cell : ({row}) => {
            const mutedBudget = !row.original.enabled;

            const amount = row.original;

            return(
                <span className = {mutedBudget ? "text-muted-foreground" : "text-foreground"}> {amount.currency} {amount.amount.toFixed(2)} </span>
            );
        },},

        {accessorKey : "window_days", header : () => "WINDOW", cell : ({row}) => (
            <span className = {!row.original.enabled ? "text-muted-foreground" : ""}> {row.original.window_days} </span>
        ),},

        {id : "actions", header : () => "ACTIONS", enableSorting : false, cell : ({row}) => (
            <div className = "flex items-center gap-2">
                <Button variant = "ghost" size = "sm" className = "h-auto p-0 text-primary" onClick = {() => edit(row.original)}> <Pencil className = "h-4 w-4"/> </Button>

                <Button variant = "ghost" size = "sm" className = "h-auto p-0 text-destructive" onClick = {() => onDelete(row.original)}> <Trash2 className = "h-4 w-4"/> </Button>
            </div>
        ),},
...
export function BudgetTable({budgets, edit, toggleEnabled, onDelete} : Readonly<PropsForBudget>){
    ...
    const forColumns = useMemo(() => helperForColumns({edit, toggleEnabled, onDelete}), [edit, toggleEnabled, onDelete],);

    const tableForBudgets = useReactTable({
        data : budgets, columns : forColumns, state : {sorting}, onSortingChange : setSorting, getCoreRowModel : getCoreRowModel(), getSortedRowModel : getSortedRowModel(),
    });
            ...
            <Table>
                ...
                <TableBody>
                    {tableForBudgets.getRowModel().rows.length === 0 ? (
                        <TableRow>
                            <TableCell colSpan = {forColumns.length} className = "py-8 text-center text-sm text-muted-foreground"> No budgets configures </TableCell>
                        </TableRow>
                    ): (
