...
public class AwsCloudConnector implements CloudConnector, UsageCapable, BillingCapable {
    ...
    public static List<String> getAllRedshiftClusters(
        ...
        try (RedshiftClient client = RedshiftClient.builder()
            ...
            software.amazon.awssdk.services.redshift.model.DescribeClustersResponse response = client
                    .describeClusters();
        ...
        for (ServiceScope serviceScope : accountScope.getServiceScopes()) { // these are for services such as EC2, RDS
            ...
            for (InstanceScope instance : serviceScope.getInstances()) { // instances within a service with a name and
                ...
                for (String instanceValue : instance.getValues()) { // the specific instance
                    Dimension dimension = Dimension.builder().name(instance.getIdentifierName()).value(instanceValue)
                            .build();

                    for (String metric : serviceScope.getMetrics()) { // the metrics requested, e.g. CPUUtilisation,
                        // NetworkIn,
                        // NetworkOut etc.
    ...
    private List<UsageRecordModel> processServiceScope(
        ...
        MutableDouble clusterState = new MutableDouble(
                50.0 + new Random(requestContext.accountSeed()).nextGaussian() * 10);
