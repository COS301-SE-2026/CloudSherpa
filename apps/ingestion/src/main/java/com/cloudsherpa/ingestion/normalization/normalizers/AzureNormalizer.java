...
@Component
public class AzureNormalizer implements Normalizer {
  private final ResourceRepository resourceRepository;

  public AzureNormalizer(ResourceRepository resourceRepository) {
  ...
  public NormalizedMetric normalize(UsageRecordModel r) {
    ...
    UUID resourceTableIdent =
        resourceRepository
            .findByAccountIdAndResourceTypeAndResourceIdentifierAndRegion(
                UUID.fromString(r.getSubscriptionId()),
                r.getServiceName(),
                r.getResourceId(),
                r.getRegion())
            .map(Resource::getId)
    ...
    String subscriptionId = r.getSubscriptionId();
