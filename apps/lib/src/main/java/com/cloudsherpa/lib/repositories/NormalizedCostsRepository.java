...
public interface NormalizedCostsRepository extends JpaRepository<NormalizedCosts, String> {
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    int upsert(@Param("entity") NormalizedCosts entity, @Param("provider") String provider,
            @Param("chargeType") String chargeType, @Param("currency") String currency);
    ...
    @Query("""
            SELECT MAX(nc.usageStartTime)
            FROM NormalizedCosts nc
            WHERE nc.provider = :provider
            """)
    Instant findLatestProviderBillingReportDate(@Param("provider") ProviderEnum provider);
