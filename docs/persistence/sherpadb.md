...
2. **Tenant Schemas (`tenant_<uuid>`):** A private, isolated database space created uniquely for each customer. These schemas hold cloud resources, time-series metrics, normalized billing data, and tenant-specific optimization data without mixing customers.
...
| **optimization_status_enum** | `DRAFT`, `ACTIVE`, `ACKNOWLEDGED`, `DISMISSED`, `APPLIED`, `SUPERSEDED`, `EXPIRED` | Lifecycle status of an optimization recommendation. |
...
These tables hold cloud resource data, time-series metrics, billing records,
and tenant-specific optimization state.
...
Optimization tables are created inside each tenant schema by
`public.create_new_tenant(...)`. They store pre-calculated metric
statistics, recommendation lifecycle data, recommendation history, and
worker processing progress.

### optimization_metric_statistics

Stores pre-calculated statistics used by optimization rules.
...
| **median_value** | NUMERIC | Median metric value. |
| **p95_value** | NUMERIC | 95th percentile metric value. |
| **p99_value** | NUMERIC | 99th percentile metric value. |
| **standard_deviation** | NUMERIC | Metric variability. |
| **spike_count** | INTEGER | Number of detected spikes. |
| **peak_duration_seconds** | INTEGER | Total time spent at peak usage. |
| **completeness_ratio** | NUMERIC | Proportion of expected data points available. |
| **window_start** | TIMESTAMPTZ | Start of the statistical window. |
| **window_end** | TIMESTAMPTZ | End of the statistical window. |
| **calculated_at** | TIMESTAMPTZ | Time when the statistics were calculated. |

### optimization_recommendation

Stores both rule-generated candidates and final recommendations. The
`status` value identifies the current stage of the recommendation lifecycle.
...
| **updated_at** | TIMESTAMPTZ | Time when the recommendation was last updated. |
| **expires_at** | TIMESTAMPTZ | Optional expiration time. |

### recommendation_history

Stores status changes for recommendations.

| Column Name | Data Type | Description |
| :--- | :--- | :--- |
| **history_id** | UUID | Primary key. |
| **recommendation_id** | UUID | Recommendation whose status changed. |
| **resource_id** | UUID | Resource associated with the recommendation. |
| **provider** | provider_enum | Cloud provider associated with the resource. |
| **rule_id** | VARCHAR(255) | Rule that generated the recommendation. |
| **action_type** | VARCHAR(50) | Recommended action. |
| **previous_status** | optimization_status_enum | Status before the change. |
| **new_status** | optimization_status_enum | Status after the change. |
| **evidence** | JSONB | Evidence associated with the status change. |
| **changed_at** | TIMESTAMPTZ | Time when the status changed. |

### processing_watermark

Tracks optimization worker progress for each processing pipeline.

| Column Name | Data Type | Description |
| :--- | :--- | :--- |
| **pipeline_name** | VARCHAR(255) | Name of the processing pipeline. |
| **last_processed_period** | TIMESTAMPTZ | Last period processed by the pipeline. |
| **last_successful_run** | TIMESTAMPTZ | Time of the last successful pipeline run. |
| **updated_at** | TIMESTAMPTZ | Time when the watermark was last updated. |
...
- Creates the `optimization_metric_statistics` table.
- Creates the `optimization_recommendation` table.
- Creates the `recommendation_history` table.
- Creates the `processing_watermark` table.
...
- Optimization metric statistics
