...
- provider (VARCHAR): AWS, AZURE, or GCP; string keeps the schema simple.
- status (VARCHAR): active, disabled, error; used to block ingestion when needed.
- created_at (TIMESTAMPTZ): creation timestamp in UTC.
...
- provider (VARCHAR): provider the credentials belong to.
- credential_type (VARCHAR): access_key, oauth, etc.
...
- account_type (VARCHAR): aws_account, azure_subscription, gcp_project.
...
- metric_type (VARCHAR): cost, usage, or performance.
